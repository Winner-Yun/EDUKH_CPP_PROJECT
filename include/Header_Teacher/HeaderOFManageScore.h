...
void MainHeadOF_ManageScore::clearScore(const char* teacherID, const char* grade) {
    ...
    if (choice != IDYES) {
        MessageBoxA(GetConsoleWindow(), "Clear operation canceled.", "Canceled", MB_OK | MB_ICONINFORMATION);
        return; 
    }
    ...
    if (anyCleared) {
        MessageBoxA(GetConsoleWindow(), "Scores cleared successfully.", "Success", MB_OK | MB_ICONINFORMATION);
    } else {
        MessageBoxA(GetConsoleWindow(), "No matching records found for this teacher and grade.", "Info", MB_OK | MB_ICONINFORMATION);
    }
}
...
void MainHeadOF_ManageScore::writeSetSchoolscore(const char* teacherID, const char* grade, const char* studentID, const char* newScore) {
    ...
    if (!found) {
        MessageBoxA(GetConsoleWindow(), "Error", "Student ID not found for this teacher and grade", MB_OK);
    }
...
void MainHeadOF_ManageScore::writeSetH2Score(const char* teacherID, const char* grade, const char* studentID, const char* newScore) {
    ...
    if (!found) {
        MessageBoxA(GetConsoleWindow(), "Error", "Matching record not found (Teacher, Grade, or StudentID mismatch)", MB_OK | MB_ICONERROR);
    }
...
void MainHeadOF_ManageScore::writeDatatoFile(const char* teacherID, const char* grade , const char* subject) {
    ...
    if (students.empty()) {
        MessageBoxA(GetConsoleWindow(), "No matching students found", "Info", MB_OK);
        return;
    }
    ...
    // 4. Merge back into full file (preserve other teachers/grades)
    {
        ...
        if (!outFile) {
            MessageBoxA(GetConsoleWindow(), "Error", "Cannot create ManageScore_data", MB_OK);
            return;
        }
